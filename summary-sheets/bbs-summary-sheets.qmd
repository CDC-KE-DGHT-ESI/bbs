...
response_rates_table <- overview_data %>%
  ...
  select(TYPOLOGY, `Sample size`, SCREENED, ELIGIBLE, CONSENTED, BLOOD_COLLECTED, INTERVIEWED) %>%
  gt() %>%
  tab_style(
  ...
  cols_label(
    TYPOLOGY = html("Key Population <br> Group"),
    ...
    BLOOD_COLLECTED = html("Consented <br> for <br> Blood"),
  ...
  opt_row_striping() %>%
  tab_footnote(
    footnote = "Potential participants with a coupon who presented at the survey site.",
...
![](`r response_rates_table_file_path `)
...
prev_data <- rio::import("data/Results/PrevalenceandVLsupression17_10_2024.xlsx",
...
supp_data <- rio::import("data/Results/PrevalenceandVLsupression17_10_2024.xlsx",
...
prev_supp_pse_table <- prevalence_suppression_pse %>%
  ...
  cols_label(
    typo = html("Key Population <br> Group"),
    prevalence = html("HIV Prevalence <br> (95% CI)"),
    suppression = html("Suppression <br> (<200 copies/ml) <br> (95% CI)"),
    pse = html("Population <br> Size Estimate <br> (95% CI)")
  ) %>%
  tab_style(
    style = list(
      cell_text(size = px(18))  # Adjust subtitle font size
...
# Save the table as an image in the specified directory
gtsave(prev_supp_pse_table, filename = prev_supp_pse_table_file_path, vwidth = 1300, vheight = 800)

```

![](`r prev_supp_pse_table_file_path `)
...
plot_height <- if (num_typos > 2) 6 else 3
...
dist_plot <- ggplot(age_dist, aes(x = factor(age), 
  ...
  geom_errorbar(aes(ymin = lcl_row/100, ymax = ucl_row/100), width = 0.2, color = "darkgray") +  
  ...
  theme(
    strip.text = element_text(face = "bold", size = 10, color = "#666666"),
...
::: {layout-ncol="1"}
![Age distribution of respondents by key population group](`r demographics_distribution_path `)
:::
...
prev_age <- rio::import("data/Results/PrevalenceandVLsupression17_10_2024.xlsx",
...
filtered_data <- prev_age %>%
  ...
  tidyr::complete(COUNTY, typo, prev_age,  
      fill = list(COUNTY = county, prev_point = 0, prev_lower = 0, prev_upper = 0))
...
if (nrow(filtered_data) > 0) {
  ...
  plot_prev <- ggplot(filtered_data, aes(x = factor(prev_age), 
                                       ...
                                       fill = typo, color = typo)) +  
  geom_bar(stat = "identity", position = position_dodge(width = 0.7), width = 0.6, color = "black") +
  geom_errorbar(aes(ymin = prev_lower, ymax = prev_upper), width = 0.2, color = "darkgray") +  
  geom_text(aes(label = ifelse(is.na(prev_point) | prev_point == 0, 0.0, 
                               format(round(prev_point * 100, 1), nsmall = 1))), 
...
supp_age <- rio::import("data/Results/PrevalenceandVLsupression17_10_2024.xlsx",
                         sheet = "Vl(200)_County_Typo_Age") |>
  mutate(typo = case_when(is.na(typo) ~ "All",
                          TRUE ~ typo),
         county = case_when(is.na(county) ~ "All",
                            TRUE ~ county) 
...
filtered_data <- supp_age %>%
  ...
  filter(!is.na(supp_point)) %>%
  tidyr::complete(COUNTY, typo, supp_age,  
      fill = list(COUNTY = county, supp_point = 0, supp_lower = 0, supp_upper = 0))
...
plot_supp_200 <- ggplot(filtered_data, aes(x = factor(supp_age), 
                                           ...
                                           fill = typo, color = typo)) +  
  geom_bar(stat = "identity", position = position_dodge(width = 0.7), width = 0.6, color = "black") +
  geom_errorbar(aes(ymin = supp_lower, ymax = supp_upper), width = 0.2, color = "darkgray") +  
  geom_text(aes(label = ifelse(is.na(supp_point) | supp_point == 0, 0.0, 
...
::: {layout-ncol="1"}
![HIV Prevalence](`r prevalence_by_kp_age_file_path `)
:::

### Viral Load suppression by Key Population Group and Age

::: {layout-ncol="1"}
![Viral load suppression](`r suppression_200_by_kp_age_file_path `)
:::
...
unconditional <- rio::import("data/Results/95_95_95/Final/results_v1.xlsx",
...
conditional <- rio::import("data/Results/95_95_95/Final/results_v1.xlsx",
...
cascade <- bind_rows(unconditional, conditional) |>
  mutate(
  lower = if_else(lower < 0, 0, if_else(lower > 100, 100, lower)),
  upper = if_else(upper < 0, 0, if_else(upper > 100, 100, upper))
  ) |>
mutate(point = point/100,
lower = lower/100,
upper = upper/100,
metric = case_when(str_detect(indicator, "ware") ~ "Aware",
str_detect(indicator, "ART") ~ "On ART",
str_detect(indicator, "pressed") ~ "Suppressed",
TRUE ~ NA_character_
)
)
...
mutate(Gap = if_else(target_value - weightedN < 0, 0, target_value - weightedN))
...
# Calculate the maximum percentage and add a buffer
max_percentage <- max(filtered_data$weightedN, na.rm = TRUE)
buffer <- min(filtered_data$weightedN, na.rm = TRUE)  # You can adjust the buffer value as needed
...
long_data <- filtered_data %>%
  pivot_longer(cols = c(weightedN, Gap), 
               names_to = "type", 
               values_to = "value") |>
mutate(
...
cascade_plot <- long_data |>
  ...
  scale_y_continuous(
    ...
    breaks = function(x) c(0, max(x)/2, max(x)),  # Show 0, middle, and max
    sec.axis = sec_axis(
      ~ normalize(.),
  ...
  geom_text(aes(label = ifelse(type == "weighted_N", 
            ...
            vjust = 0,  # Use vjust = 0 to align text with the base
            color = "black", size = 3.5, fontface = "bold") + 
  theme_minimal() +
  labs(x = "Metric", y = "Estimated population", fill = "Category") +
  facet_wrap(~ typo, nrow = facet_rows, scales = "free") +
  # Apply the custom color palette and include the gap explanation
  scale_fill_manual(
    values = c(
      "Gap" = "lightgray",
    ...
    labels = c(
      ...
      "On ART" = "On Treatment",
      "Suppressed" = "Viral Load Suppressed"
    )
  ) +
  theme(
...
# ::: {layout-ncol="1"}
# ![95-95-95 Cascade](`r cascade_by_kp_age_file_path `)
# :::
...
# Aware data
aware_unconditional <- read_csv("data/Results/95_95_95/aware_nonconditional-2024-10-01.csv") |>
rename(typo = Typology,
county = County) %>%
...
rename(indicator = aware_) |>
mutate(report = "Unconditional")


aware_conditional <- read_csv("data/Results/95_95_95/aware_conditional-2024-10-01.csv") |>
rename(typo = Typology,
county = County) %>%
# separate(prev_typo, c("county","typo")) |>
  mutate(typo = case_when(is.na(typo) ~ "All",
...
aware <- bind_rows(aware_conditional, aware_unconditional)
...
# On ARVs
onarvs_unconditional <- read_csv("data/Results/95_95_95/revised/onarvs_nonconditional-2024-10-23.csv") |>
rename(typo = typology,
county = county) %>%
# separate(prev_typo, c("county","typo")) |>
  mutate(typo = case_when(is.na(typo) ~ "All",
...
rename(indicator = onarvs_) |>
mutate(report = "Unconditional")


onarvs_conditional <- read_csv("data/Results/95_95_95/revised/onarvs_conditional-2024-10-23.csv") |>
rename(typo = typology,
county = county) %>%
# separate(prev_typo, c("county","typo")) |>
  mutate(typo = case_when(is.na(typo) ~ "All",
...
onarvs <- bind_rows(onarvs_conditional, onarvs_unconditional)
...
# Suppressed 200 copies/ml - National guidelines
vls200_unconditional <- read_csv("data/Results/95_95_95/revised/vls200_nonconditional-2024-10-23.csv") |>
rename(typo = typology,
county = county) %>%
# separate(prev_typo, c("county","typo")) |>
  mutate(typo = case_when(is.na(typo) ~ "All",
...
rename(indicator = vls_200) |>
mutate(report = "Unconditional")


vls200_conditional <- read_csv("data/Results/95_95_95/revised/vls200_conditional-2024-10-23.csv") |>
rename(typo = typology,
county = county) %>%
# separate(prev_typo, c("county","typo")) |>
  mutate(typo = case_when(is.na(typo) ~ "All",
...
vls200 <- bind_rows(vls200_conditional, vls200_unconditional)
```
...
cascade_data <- bind_rows(aware, onarvs, vls200) |>
  mutate(
  lower = if_else(lower < 0, 0, if_else(lower > 100, 100, lower)),
  upper = if_else(upper < 0, 0, if_else(upper > 100, 100, upper))
  ) |>
...
# Define the order of the age groups
metric_order <- unique(cascade_data$metric)


# Filter and prepare the data
filtered_data <- cascade_data %>%
  rename(COUNTY = county) %>%
  filter(COUNTY == county) |>
  filter(report == "Unconditional") 


filtered_conditional <- cascade_data %>%
  rename(COUNTY = county) %>%
  filter(COUNTY == county) |>
  filter(report == "Conditional") %>%
  select(COUNTY, typo, indicator, point, lower, upper) |>
...
filtered_data_labels <- cascade_data %>%
  rename(COUNTY = county) %>%
  filter(COUNTY == county) |>
  filter(report == "Conditional")
...
cascade_plot <- long_data |>
  ...
  scale_y_continuous(
    labels = scales::label_comma(),
    breaks = function(x) c(0, max(x)/2, max(x)),  # Show 0, middle, and max
    sec.axis = sec_axis(~ normalize(.),
                        name = element_blank(),
                        breaks = c(0, 1),  # Shows only 0%, and 100%
                        labels = scales::label_percent())
  ) +
  # geom_errorbar(aes(ymin = ifelse(type == "weighted_N", lower*value, 0),
  #                   ymax = ifelse(type == "weighted_N", upper*value, 0)),
  #                   width = 0.2,
  #   color = "darkgray") +
  geom_text(aes(label = ifelse(type == "weighted_N", 
                     scales::label_percent(accuracy = 0.1)(cond_point), "")),
            vjust = 0.5, color = "black", size = 4, facefont = "bold",
            position = position_stack(vjust = 0.5)) +
  theme_minimal() +
  labs(x = "Metric", y = "Estimated population", fill = "Category") +
  facet_wrap(~ typo, scales = "free_y") +
  # Apply the custom color palette
  scale_fill_manual(values = custom_colors_cascade) +
  theme(
...
if (nrow(sti_data_pos) > 0) {
  ...
  geom_text(aes(label = format(round(point * 100, 1), nsmall = 1)), 
            position = position_dodge(width = 0.75), 
...
::: {layout-ncol="1"}
![STI prevalence](`r sti_by_kp_age_file_path `)
...
stigma_long <- stigma %>%
 ...
 metric = case_when(
  ...
  str_detect(indicator, "scolded") ~ "Ever Scolded",
  str_detect(indicator, "blackmailed") ~ "Ever Blackmailed",
...
if (nrow(filtered_data) > 0) {
  ...
  plot_stigma <- ggplot(filtered_data, aes(y = factor(metric, levels = metric_order), 
    ...
    theme_minimal() +
    geom_text(aes(label = format(round(point*100,1), nsmall = 1)), 
              position = position_dodge(width = 0.9), hjust = 1.0, 
    ...
    scale_y_discrete(labels = function(labels) scales::label_wrap(40)(labels))
...
::: {layout-ncol="1"}
![Stigma and discrimination](`r stigma_by_kp_age_file_path `)
:::
...
plot_avoidhc <- ggplot(filtered_data, aes(y = factor(metric, levels = metric_order), 
    ...
    scale_y_discrete(labels = function(labels) scales::label_wrap(30)(labels))
...
::: {layout-ncol="1"}
![Avoiding Healthcare](`r avoidhc_by_kp_age_file_path `)
:::
...
if (nrow(filtered_data) > 0) {
  ...
  if (num_typos == 1) {
    ...
    plot_condoms <- ggplot(filtered_data, aes(x = typo, y = point, fill = typo, color = typo)) +
      ...
      geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.1, 
                    color = "darkgray", position = position_dodge(width = 0.75)) +
      scale_y_continuous(labels = scales::label_percent(accuracy = 1), 
                         limits = c(0, max_percentage + buffer)) +
      geom_text(aes(label = scales::label_percent(accuracy = 0.1)(point)), 
                position = position_dodge(width = 0.9), color = "black", vjust = 1.5,
...
![Access to free condoms](`r condoms_by_kp_age_file_path `)
:::
...
if (nrow(condom_use) > 0) {
  ...
  plot_condom_use <- ggplot(condom_use, aes(x = typo, 
                                          y = point,
                                          fill = partner)) +
  scale_y_continuous(labels = scales::label_percent(accuracy = 1)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.7), width = 0.6, color = "black") +  
  ...
  geom_text(aes(label = format(round(point * 100, 1), nsmall = 1)), 
            position = position_dodge(width = 0.75), 
            vjust = -0.5, color = "black", size = 4.0, fontface = "bold") +
  labs(y = element_blank(),
       x = element_blank()) +
  theme_minimal() +
  theme(
...
![Condom usage in the last sexual encounter](`r condom_use_file_path `)
:::
...
num_typos <- filtered_data %>% distinct(typo) %>% pull(typo) %>% length()
plot_height <- if (num_typos > 2) 4 else 2
plot_width <- if (num_typos > 1) 8 else 4
...
if (nrow(filtered_data) > 0) {
  ...
  plot_prep <- ggplot(filtered_data, aes(y = metric, 
    ...
    theme_minimal() +
    geom_text(aes(label = scales::label_percent(accuracy = 0.1)(point)), 
              position = position_dodge(width = 0.9), hjust = -0.5, color = "black", size = 4,fontface = "bold") +
    scale_fill_manual(values = custom_colors) +
    scale_color_manual(values = custom_colors) +  
    theme(
...
::: {layout-ncol="1"}
![PrEP knowledge and use](`r prep_by_kp_age_file_path `)
:::

### PWID - Medication-Assisted Treatment
...
oat <- read_csv("data/Results/Prevention/2024-10-09/oat-report.csv") |>
  mutate(typo = case_when(is.na(typo) ~ "All",
                          TRUE ~ typo),
         county = case_when(is.na(county) ~ "All",
                            TRUE ~ county) 
...
oat_long <- oat %>%
  ...
  metric = case_when(
      str_detect(indicator, "oat_aware_of_drug_tx_prog") ~ "Aware of drug Tx program",
      str_detect(indicator, "oat_ever_been_drug_tx_prog") ~ "Ever been on a drug Tx program",
      str_detect(indicator, "oat_in_drug_tx_prog_12mon") ~ "In a drug Tx program < 12 months",
      TRUE ~ NA_character_
    )
) %>%
filter(!is.na(metric)) %>%
...
if (nrow(filtered_data) > 0) {
  ...
  # Create the plot
  plot_oat <- ggplot(filtered_data, aes(y = population, 
                                         x = metric,
                                         fill = typo, color = typo)) +
    geom_bar(stat = "identity", position = position_dodge(width = 0.6), width = 0.7, color = "black") +
    facet_wrap(~ typo,  ncol = 2, scales = "fixed") +
    labs(y = element_blank(),
         x = element_blank()) +
    theme_minimal() +
    geom_text(aes(label = scales::label_percent(accuracy = 0.1)(point)), 
              position = position_dodge(width = 0.7), color = "black", size = 3.5,fontface = "bold", vjust = 1.5) +
    scale_fill_manual(values = custom_colors) +
    scale_color_manual(values = custom_colors) +  
    theme(
      #strip.background = element_rect(fill = "#E0E0E0", color = "black"),  
...
::: {layout-ncol="1"}
![Medication-Assisted Treatment](`r oat_by_kp_age_file_path `)
:::
...
hts <- read_csv("data/Results/Prevention/2024-10-09/hx-report.csv") |>
  mutate(typo = case_when(is.na(typo) ~ "All",
                          TRUE ~ typo),
         county = case_when(is.na(county) ~ "All",
                            TRUE ~ county) 
...
hts_long <- hts %>%
  ...
  metric = case_when(
      ...
      str_detect(indicator, "less_3mths") ~ "Within the last 3 months)",
      str_detect(indicator, "4_6_mths") ~ "Within 4-6 months go",
      str_detect(indicator, "7_12_mths") ~ "Within 7-12 months ago",
      str_detect(indicator, "12_plus") ~ "Over 12 months ago",
      TRUE ~ NA_character_
    )
) %>%
select(-indicator) %>%
...
num_typos <- filtered_data %>% distinct(typo) %>% pull(typo) %>% length()
plot_height <- if (num_typos > 3) 4 else 2
plot_width <- if (num_typos > 1) 8 else 4
...
if (nrow(filtered_data) > 0) {
  ...
  # Create the plot
  plot_hts <- ggplot(filtered_data, aes(y = factor(metric, levels = metric_order), 
                                         x = point,
                                         fill = typo, color = typo)) +
    scale_x_continuous(labels = scales::label_percent(accuracy = 1), 
                       limits = c(0, max_percentage + buffer)) +
...
::: {layout-ncol="1"}
![HIV Testing](`r hts_by_kp_age_file_path `)
:::
...
![Contact with outreach worker](`r peer_by_kp_age_file_path `)
:::
